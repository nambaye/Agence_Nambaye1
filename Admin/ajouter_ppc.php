...
if (isset($_REQUEST['ajouter'])) {
  if(empty($_POST['description']) OR empty($_POST['chambre']) OR empty($_POST['salon']) OR empty($_POST['douche_interne']) OR empty($_POST['douche_externe']) OR empty($_POST['garage']) OR empty($_POST['cuisine_interne']) OR empty($_POST['salle_manger']) OR empty($_POST['delai_construction']) OR empty($_POST['prix_devis'])){

      //Code JavaScript
        ?>
        <script language="javascript">
            window.alert('Tous les champs doivent être completés !')
            document.location.href="ajouter_ppc.php"
        </script>
        <?php
        exit();
      ...
      $dim = addslashes($_POST['dimension']);
      ...
      $cuisine_interne = addslashes($_POST['cuisine_interne']);
      $salle_manger = addslashes($_POST['salle_manger']);
      $delai_construction = addslashes($_POST['delai_construction']);
      $prix_devis = addslashes($_POST['prix_devis']);
      ...
      $sql = "INSERT INTO `ppc` (`description`,`dimension`, `chambre`, `salon`, `douche_interne`, `garage`, `cuisine_interne`, `salle_manger`, `delai_construction`, `prix_devis`, `img1`, `img2`, `img3`, `img4`) VALUES ('$des','$dim', '$chambre', '$salon', '$douche_interne', '$garage', '$cuisine_interne', '$salle_manger', '$delai_construction', '$prix_devis', '$imge1', '$imge2', '$imge3', '$imge4') " ;

      $res = $con->query($sql) or die ("Erreur sql : $sql<br/>".mysqli_error($con));
      move_uploaded_file($temp_name1,"../images/image_ppc/".$imge1);
      move_uploaded_file($temp_name2,"../images/image_ppc/".$imge2);
      move_uploaded_file($temp_name3,"../images/image_ppc/".$imge3);
      move_uploaded_file($temp_name4,"../images/image_ppc/".$imge4);

      if($res){ 
        //Code JavaScript
        ?>
          <script language="javascript">
          window.alert("PPC publié avec succès !")
          document.location.href="ajouter_ppc.php"
          </script>
        <?php
  ...
  <section id="container">
    ...
    <!-- **********************************************************************************************************************************************************
      ...
      <form class="form-login1" action="" method="POST" enctype="multipart/form-data">
        <h2 class="form-login-heading1">Ajouter PPC !</h2>
        ...
        <div class="row">
         <div class="col-md-6">
          ...
          <input type="text" class="form-control" placeholder="" autofocus name="dimension">

          <label>Chambre</label>
          <input type="number" min="1" class="form-control" placeholder="" autofocus name="chambre">

          <label>Salon</label>
          <input type="nuumber" min="1" class="form-control" placeholder="" autofocus name="salon">

          <label>Douche interne</label>
          <input type="number" min="0" class="form-control" placeholder="" autofocus name="douche_interne">

          <label>Garage</label>
          <input type="number" min="0" class="form-control" placeholder="" autofocus name="garage">

          <label>Cuisine interne</label>
          <input type="number" min="0" class="form-control" placeholder="" autofocus name="cuisine_interne">
          <br>
        </div>
          
        <div class="col-md-6">
          <label>Salle à manger</label>
          <input type="number" min="0" class="form-control" placeholder="" name="salle_manger">

          <label>Delai construction</label>
          <input type="text" class="form-control" placeholder="" autofocus name="delai_construction">

          <label>Prix devis</label>
          <input type="text" class="form-control" placeholder="" autofocus name="prix_devis">
